...
CAN_Interrupt_Callbacks CAN_TxMailboxCompleteCallback[3] = {
		&CAN_TxMailbox0CompleteCallback, &CAN_TxMailbox1CompleteCallback,
...
void CAN_voidGetRxMsg(u32 local_u8RxFifo, CAN_RxHeaderTypeDef *pRxHeader,
		u8 Local_u8Data[]) {
	/* Get Identifier extension */
	pRxHeader->IDE = READ_BIT(CAN->FIFOMailBox[local_u8RxFifo].RIR, 2);
	if (pRxHeader->IDE == CAN_ID_STD) {/* GET the Id */
		pRxHeader->StdId = CAN->FIFOMailBox[local_u8RxFifo].RIR >> 21;
	} else {
		pRxHeader->ExtId = CAN->FIFOMailBox[local_u8RxFifo].RIR >> 3;
	}
	/* GET the RTR */
	pRxHeader->RTR = READ_BIT(CAN->FIFOMailBox[local_u8RxFifo].RIR, 1);
	/* GET the DLC */
	pRxHeader->DLC = (CAN->FIFOMailBox[local_u8RxFifo].RDTR & 0xF);
	/* GET the (FMI) Filter Match Index */
	pRxHeader->FilterMatchIndex = (CAN->FIFOMailBox[local_u8RxFifo].RDTR >> 8)
			& 0xFF;
	/* GET the Time stamp */
	pRxHeader->TimeStamp = (CAN->FIFOMailBox[local_u8RxFifo].RDTR >> 16)
			& 0XFFFF;
    ...
    Local_u8Data[7] = (u8)((CAN->FIFOMailBox[local_u8RxFifo].RDHR >> 24) & 0XFF);
		#else
	for (u8 i = 0; i < 8; i++) {/*clear buff*/
		Local_u8Data[i] = 0;
	}
	for (u8 i = 0; i < pRxHeader->DLC; i++) {
		if (i < 4)
			Local_u8Data[i] = (u8) ((CAN->FIFOMailBox[local_u8RxFifo].RDLR
					>> (i * 8)) & 0XFF);
		else
			Local_u8Data[i] = (u8) ((CAN->FIFOMailBox[local_u8RxFifo].RDHR
					>> ((i - 4) * 8)) & 0XFF);
	}
...
void CAN_voidTX_Interrupt_Handler(void) {
	...
	for (u8 local_u8TxBitPos = 0; local_u8TxBitPos <= 16; local_u8TxBitPos += 8) // local_u8TxBitPos refer to Position of RQCPx bit in TSR Register
			{
		if (READ_BIT(tsrflags, (local_u8TxBitPos)) != 0) /* RQCP: Request completed mailbox [0:2] */
		{
			...
			if (READ_BIT(tsrflags, (local_u8TxBitPos + 1)) != 0) /* TXOK: Transmission OK of mailbox [0:2] */
			{
				...
				if (READ_BIT(tsrflags, (local_u8TxBitPos + 2)) != 0) /* ALST: Arbitration lost for mailbox [0:2] */
				{
					/* Update error code */
				...
				} else if (READ_BIT(tsrflags, (local_u8TxBitPos + 3)) != 0) /* TERR: Transmission error of mailbox [0:2] */
				{
					/* Update error code */
...
void CAN_voidRX_FIFO0_Interrupt_Handler(void) {
	...
	if (READ_BIT(interrupts, 3) != 0) /* FOVIE0: FIFO overrun interrupt enable */
	{
		if (READ_BIT(rf0rflags, 4) != 0) /* FOVR0: FIFO 0 overrun */
		{
	...
	if (READ_BIT(interrupts, 2) != 0) /* FFIE0: FIFO full interrupt enable */
	{
		if (READ_BIT(rf0rflags, 3) != 0) /* FULL0: FIFO 0 full */
		{
	...
	if (READ_BIT(interrupts, 1) != 0) /* FMPIE0: FIFO message pending interrupt enable */
	{
...
void CAN_voidRX_FIFO1_Interrupt_Handler(void) {
	...
	if (READ_BIT(interrupts, 6) != 0) /* FOVIE1: FIFO overrun interrupt enable */
	{
		if (READ_BIT(rf1rflags, 4) != 0) /* FOVR1: FIFO 1 overrun */
		{
	...
	if (READ_BIT(interrupts, 5) != 0) /* FFIE1: FIFO full interrupt enable */
	{
		if (READ_BIT(rf1rflags, 3) != 0) /* FULL1: FIFO 1 full */
		{
	...
	if (READ_BIT(interrupts, 4) != 0) /* FMPIE1: FIFO message pending interrupt enable */
	{
		/* Check if message is still pending */
...
void CAN_voidSCE_Interrupt_Handler(void) {
	...
	if (READ_BIT(interrupts, 17) != 0) /* SLKIE: Sleep interrupt enable */
	{
		if (READ_BIT(msrflags, 4) != 0) /* SLAKI: Sleep acknowledge interrupt */
		{
	...
	if (READ_BIT(interrupts, 16) != 0) /* WKUIE: Wakeup interrupt enable */
	{
		if (READ_BIT(msrflags, 3) != 0) /* WKUI: Wakeup interrupt */
		{
			/* Clear WakeUp Flag */
	...
	if (READ_BIT(interrupts, 15) != 0) /* ERRIE: Error interrupt enable */
	{
		if (READ_BIT(msrflags, 2) != 0) /* ERRI: Error interrupt */
		{
			/* Check Error Warning Flag */
			if ((READ_BIT(interrupts, 8) != 0) && /* EWGIE: Error warning interrupt enable*/
			(READ_BIT(esrflags, 0) != 0)) /* EWGF: Error warning flag */
			{
			...
			if ((READ_BIT(interrupts, 9) != 0) && /* EPVIE: Error passive interrupt enable*/
			(READ_BIT(esrflags, 1) != 0)) /* EPVF: Error passive flag */
			{
			...
			/* Check Bus-off Flag */
			if ((READ_BIT(interrupts, 10) != 0) && /* BOFIE: Bus-off interrupt enable */
			(READ_BIT(esrflags, 2) != 0)) /* BOFF: Bus-off flag */
			{
			...
			if ((READ_BIT(interrupts, 11) != 0) && /* LECIE: Last error code interrupt enable */
